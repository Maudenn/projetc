...
public class VilleFactory {
	...
	public static Ville create(String fichier){
		try{
			...
			Ville v = new Ville(Integer.parseInt(s[2]), Integer.parseInt(s[3]),Integer.parseInt(s[0]), Integer.parseInt(s[4]));
			// suite
			int i = 0;
	...
	public static MatriceVille createM(String fichier){
		try{
			InputStream ips=new FileInputStream(fichier);
			InputStreamReader ipsr=new InputStreamReader(ips);
			BufferedReader br=new BufferedReader(ipsr);
			...
			ligne=br.readLine();
			String[] s = ligne.split(" ");
			MatriceVille v = new MatriceVille(Integer.parseInt(s[2]), Integer.parseInt(s[3]),Integer.parseInt(s[0]), Integer.parseInt(s[4]));
			// suite
			while ((ligne=br.readLine())!=null){
				s = ligne.split(" ");
				if(s.length==2){
